...
/// Cliente REST del centro de ayuda del conductor.
class SupportApi {
  SupportApi([DioClient? client]) : _client = client ?? DioClient();
  final DioClient _client;

  Future<List<SupportTicket>> list() async {
    final res = await _client.get<Map<String, dynamic>>('/driver/support/tickets');
  ...
  Future<SupportTicket> detail(String id) async {
    final res = await _client.get<Map<String, dynamic>>('/driver/support/tickets/$id');
    return SupportTicket.fromJson(res.data!['data'] as Map<String, dynamic>);
  }
