...
## PENDIENTE (Tanda 6) — punteros exactos
...
2. **`Order.operatorId`:** sellarlo cuando el backend tenga despacho/accept de pedidos por conductores (hoy `Order.driverId` nunca se asigna server-side).
...
4. **Colores inline fuera de intercity:** `transport_home_screen` del cliente aún tiene ~24 `Color(0x…)` (home funcional; barrido opcional).
