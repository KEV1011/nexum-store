...
## PENDIENTE (Tanda 8) — punteros exactos
...
3. **Foto de recogida/entrega del pedido subida al backend:** hoy el conductor captura la prueba (DeliveryProofSheet) pero para pedidos no se sube la imagen a `Order.pickupPhotoUrl/deliveryPhotoUrl` (viajes/envíos sí). Falta el multipart en `order_status`/una ruta.
