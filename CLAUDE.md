...
## PENDIENTE (Tanda 5) — punteros exactos
...
3. **Portal de negocio del app conductor** (`AppTransport/features/business_portal`, datasource 100 % mock, hoy oculto en release): decidir si se elimina o se conecta al backend de negocios real (la web `/negocio` ya es real).
4. **Código legacy por retirar:** `features/documents` (pantalla con `_simulateUpload`, ya desenlazada del drawer), `features/trip_requests` (datasource mock aún instanciado — ¿alguna pantalla lo usa?), mocks `core/mock_data/{passengers,trips,errands}_mock.dart` (solo los importa trip_requests_datasource).
5. **Pulido visual restante:** pantallas intercity de ambas apps usan ~40 `Color(0x…)` inline en vez de `AppColors` (deriva en dark mode); sonido de notificación TODO en `notification_service.dart`.
