...
class _DeleteAccountScreenState extends ConsumerState<DeleteAccountScreen> {
  ...
  Future<void> _comprobar() async {
    try {
      final res = await ref
          ...
          .get<Map<String, dynamic>>('/client/account/deletion');
  ...
  Future<void> _eliminar() async {
    ...
    try {
      await ref.read(apiClientProvider).delete<Map<String, dynamic>>('/client/account');
      if (!mounted) return;
  ...
  Future<void> _confirmar() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Esto no se puede deshacer. Perderás tu historial, tus direcciones '
          'guardadas y tus cupones.',
                ...
                if (_error != null) ...[
                  ...
                  const _Punto(
                    ...
                    detalle:
                        ...
                        'te registras de nuevo y empiezas desde cero.',
                  ),
                  const _Punto(
                    icono: Icons.delete_outline_rounded,
                    titulo: 'Borramos tus datos personales',
                    detalle:
                        'Nombre, correo, foto, direcciones guardadas y tu '
                        'contacto de confianza.',
                  ),
                  const _Punto(
                    icono: Icons.receipt_long_outlined,
                    titulo: 'Los viajes ya pagados se conservan',
                    detalle:
                        'Sin tu nombre ni tu teléfono. Llevan lo que cobró el '
                        'conductor y su comisión: es la contabilidad de él y de '
                        'su empresa, y por ley hay que guardarla.',
                  ),
                  const SizedBox(height: AppConstants.spacingL),
