...
class _TripChatScreenState extends ConsumerState<TripChatScreen> {
  ...
  void _onEvent(TripChatEvent e) {
    ...
    if (e.history != null) {
      setState(() {
        _messages
          ..clear()
          ..addAll(e.history!.map(_TripMsg.fromJson));
      });
      _scrollToBottom();
    } else if (e.message != null) {
      final m = _TripMsg.fromJson(e.message!);
  ...
  Future<void> _sendPhoto() async {
    ...
    try {
      ...
      // "No se pudo enviar la foto" a secas era un `catch (_)` mudo: si el
      // backend rechaza por autorización, tamaño o tipo de archivo, el
      // motivo real se perdía y no había forma de diagnosticar el fallo
      // sin acceso al servidor. El backend explica POR QUÉ, igual que ya
      // se hace al crear un pedido.
      if (mounted) {
        ...
        children: [
          Expanded(
            child: _messages.isEmpty
                ? Center(
                    child: Text(
                      'Escribe para coordinar con tu conductor.',
