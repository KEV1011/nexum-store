...
class ModeracionApi {
  ModeracionApi(this._dio, {this.basePath = '/client'});
  final Dio _dio;
  final String basePath;

  Future<List<MotivoReporte>> motivos() async {
    final res = await _dio.get<Map<String, dynamic>>('$basePath/reports/reasons');
  ...
  Future<void> reportar({
    ...
    await _dio.post<Map<String, dynamic>>('$basePath/reports', data: {
      'targetKind': tipo,
      'targetId': objetivoId,
      'reason': motivo,
  ...
  Future<void> bloquear({required String tipo, required String id, String? motivo}) async {
    await _dio.post<Map<String, dynamic>>('$basePath/blocks', data: {
      'kind': tipo,
      'id': id,
  ...
  Future<void> desbloquear(String personaId) async {
    await _dio.delete<Map<String, dynamic>>('$basePath/blocks/$personaId');
  }

  Future<List<Bloqueado>> bloqueados() async {
    final res = await _dio.get<Map<String, dynamic>>('$basePath/blocks');
    return (res.data?['data'] as List<dynamic>? ?? [])
        .whereType<Map<String, dynamic>>()
...
final moderacionApiProvider = Provider<ModeracionApi>((ref) {
  return ModeracionApi(ref.read(apiClientProvider));
});
